...
class OrganisationServiceTest(django.test.TestCase):
    ...
    def setUp(self):
        ...
        OrganisationMembership.objects.create(
            user=self.manager1, organisation=self.org, role=ROLE_ADMIN
        )
    ...
    def test_create_organisation(self):
        ...
        org_membership = OrganisationMembership.objects.filter(
            user=manager, organisation=created_org
        ).first()
    ...
    def test_update_organisation(self):
        ...
        with self.assertRaises(PermissionDenied):
            organisation_service.update_organisation(
                self.manager2, self.org, new_values
            )
    ...
    def test_add_user_to_organisation(self):
        ...
        membership = OrganisationMembership.objects.filter(
            user=self.manager2, organisation=self.org
        ).first()
    ...
    def test_remove_user_from_organisation(self):
        ...
        OrganisationMembership.objects.create(
            user=self.manager3, organisation=self.org, role=ROLE_PROJECT_MANAGER
        )
        self.assertEqual(
            OrganisationMembership.objects.filter(
                user=self.manager3, organisation=self.org
            ).count(),
            1,
        )
        # Remove manager 3 then check
        organisation_service.remove_user_from_organisation(
            self.manager1, self.org, self.manager3
        )
        self.assertEqual(
            OrganisationMembership.objects.filter(
                user=self.manager3, organisation=self.org
        ...
        with self.assertRaises(PermissionDenied):
            organisation_service.remove_user_from_organisation(
                self.manager2, self.org, self.manager1
            )

    def test_get_organisation_members(self):
        self.assertEqual(
            organisation_service.get_organisation_members(
                self.manager1, self.org
            ).count(),
            1,
        )
...
class ServicesPermissionsTest(django.test.TestCase):
    ...
    def test_organisation_view_permission(self):
        self.assertTrue(
            organisation_service.can_view(self.manager1, self.org),
            "Manager cannot view own org",
        )
        self.assertFalse(
            organisation_service.can_view(self.manager2, self.org),
            "Manager can view other org",
        )

    def test_organisation_create_permission(self):
        self.assertFalse(
            organisation_service.can_create(self.manager1),
            "Manager can create another org when they're a member of an existing one",
        )
        self.assertTrue(
            organisation_service.can_create(self.manager2), "Manager cannot create org"
        )

    def test_organisation_edit_permission(self):
        self.assertTrue(
            organisation_service.can_edit(self.manager1, self.org),
            "Manager cannot edit own org",
        )
        self.assertFalse(
            organisation_service.can_edit(self.manager2, self.org),
            "Manager can edit other org",
        )

    def test_organisation_delete_permission(self):
        self.assertTrue(
            organisation_service.can_delete(self.manager1, self.org),
            "Manager cannot delete own org",
        )
        self.assertFalse(
            organisation_service.can_delete(self.manager2, self.org),
            "Manager can delete other org",
        )

    def test_project_view_permission(self):
        self.assertTrue(
            project_service.can_view(self.manager1, self.project),
            "Manager cannot view own project",
        )
        self.assertFalse(
            project_service.can_view(self.manager2, self.project),
            "Manager can view other project",
        )

    def test_project_create_permission(self):
        self.assertTrue(
            project_service.can_create(self.manager1, self.org),
            "Manager cannot create project in org they belong to",
        )
        self.assertFalse(
            project_service.can_create(self.manager2, self.org),
            "Manager can create project in other org",
        )

    def test_project_edit_permission(self):
        self.assertTrue(
            project_service.can_edit(self.manager1, self.project),
            "Manager cannot edit own project",
        )
        self.assertFalse(
            project_service.can_edit(self.manager2, self.project),
            "Manager can edit other project",
        )

    def test_project_delete_permission(self):
        self.assertTrue(
            project_service.can_delete(self.manager1, self.project),
            "Manager cannot delete own project",
        )
        self.assertFalse(
            project_service.can_delete(self.manager2, self.project),
            "Manager can delete other project",
        )

    def test_survey_view_permission(self):
        self.assertTrue(
            survey_service.can_view(self.manager1, self.survey),
            "Manager cannot view own survey",
        )
        self.assertFalse(
            survey_service.can_view(self.manager2, self.survey),
            "Manager can view other survey",
        )

    def test_survey_create_permission(self):
        self.assertTrue(
            survey_service.can_create(self.manager1, self.project),
            "Manager cannot create survey in org they belong to",
        )
        self.assertFalse(
            survey_service.can_create(self.manager2, self.project),
            "Manager can create survey in other org",
        )

    def test_survey_edit_permission(self):
        self.assertTrue(
            survey_service.can_edit(self.manager1, self.survey),
            "Manager cannot edit own survey",
        )
        self.assertFalse(
            survey_service.can_edit(self.manager2, self.survey),
            "Manager can edit other survey",
        )

    def test_survey_delete_permission(self):
        self.assertTrue(
            survey_service.can_delete(self.manager1, self.survey),
            "Manager cannot delete own survey",
        )
        self.assertFalse(
            survey_service.can_delete(self.manager2, self.survey),
            "Manager can delete other survey",
        )
