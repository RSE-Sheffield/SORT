<script lang="ts">
  ...
  function handleMoveRequest(
    ...
    if (srcSectionIndex === destSectionIndex && destFieldIndex < 0) return; // Don't move if dropped into the same section

    if (destFieldIndex >= 0) {
      // Move within or between sections with existing elements
      let fieldItem = _.cloneDeep(
        config.sections[srcSectionIndex].fields[srcFieldIndex],
      );
      config.sections[srcSectionIndex].fields.splice(srcFieldIndex, 1);
      config.sections[destSectionIndex].fields.splice(
        destFieldIndex,
        0,
        fieldItem,
      );
      ...
      config.sections[srcSectionIndex].fields.splice(srcFieldIndex, 1);
      config.sections[destSectionIndex].fields = [
        ...config.sections[destSectionIndex].fields,
        fieldItem,
      ];
    }
