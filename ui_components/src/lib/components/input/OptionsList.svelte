...
{#each options as option, index (index)}
    <div class="input-group mb-1">
        ...
        {#if readonly}
            <button class="btn btn-outline-secondary" aria-label="Delete option" title="Delete option"><i
                    class='bx bx-x'></i></button>
        {:else}
            <button class="btn btn-outline-danger" onclick={()=>{deleteOption(index)}} aria-label="Delete option"
                    title="Delete option"><i class='bx bx-x'></i></button>
        {/if}
    </div>
{/each}
{#if type === "radio" && hasOtherOption}
    <div class="input-group mb-1">
        ...
        <input class="form-control" type="text" value="" readonly={true} disabled={true}
               placeholder="Custom text can be added by participants."/>
        {#if readonly}
            <button class="btn btn-outline-secondary" aria-label="Delete option" title="Delete option"><i
                    class='bx bx-x'></i></button>
        {:else}
            <button class="btn btn-outline-danger" onclick={()=>{setHasOtherOption(false)}} aria-label="Delete option"
                    title="Delete option"><i class='bx bx-x'></i></button>
        {/if}
    </div>
{/if}
<div>
    ...
    {:else}
        ...
        {#if type === "radio" && !hasOtherOption}
            or
            <button class="btn btn-primary btn-sm" onclick={()=>{setHasOtherOption(true)}}><i class="bx bx-plus"></i>
                Add "Other" option
            </button>
        {/if}
    {/if}
